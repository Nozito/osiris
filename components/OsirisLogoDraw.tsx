...
// Traced Osiris mark — single path, viewBox 402.021x374.505.
const OSIRIS_PATH =
...
/**
 * The Osiris mark as a vector line animation: the outline strokes itself
 * in, then the shape solidifies with a fill (and the reverse for "out").
 */
export const OsirisLogoDraw: React.FC<{
  play: 'in' | 'out';
  size?: number;
  className?: string;
}> = ({ play, size = 96, className = '' }) => {
    ...
    >
      <path
        ...
        style={{
          ...
          transition: active
            ? `stroke-dashoffset 1900ms ${EASE_PREMIUM}`
            : `stroke-dashoffset 1700ms ${EASE_PREMIUM} 350ms`,
        }}
      />
      <path
        d={OSIRIS_PATH}
        ...
        style={{
          opacity: active ? 1 : 0,
          transition: active ? 'opacity 550ms ease 1500ms' : 'opacity 350ms ease',
        }}
      />
    </svg>
  );
